...
class DisputeToDB:
    ...
    def updateFailedCaseToMDB(self, iTNums):
        ...
        for i in range( iTNums ):
            update_failed_case[ 'update_failed_case_thread_' + str( i + 1 ) ] = UpdateFailedCaseTask( ( i + 1 ), "update_failed_case_thread_" + str( i + 1 ) );
    ...
    def execDisputeToDB(self, sid=None, iTNums=None):
        ...
        if iThreadNums > 0:
            for i in range( iThreadNums ):
                save_disputes[ 'save_disputes_thread_' + str( i + 1 ) ] = SaveDisputesTask( ( i + 1 ), "save_disputes_thread_" + str( i + 1 ), iSiteId );
