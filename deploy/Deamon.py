...
class Task( CDaemon ):
    redisConn = None;
    siteCaseQueue = 'l_update_site_queue';
    sleepTime = 10;
    iThreadNums = 5;
    ...
    def updateSiteCase( self, iSiteId ):
        #执行拉取列表的任务
        GetDisputesList().execGetDisputes( iSiteId );
        #将redis中的列表数据取出写入到mysqlDB
        DisputeToDB().execDisputeToDB( iSiteId, self.iThreadNums );
    
    def run( self, output_fn, **kwargs ):
        #fd = open(output_fn, 'w');
        
        while True:
            if self.redisConn.llen( self.siteCaseQueue ) > 0:
                iSiteId = self.redisConn.rpop( self.siteCaseQueue );
                self.updateSiteCase( int( str(iSiteId,encoding='utf-8') ) );
...
if __name__ == '__main__':
    ...
    p_name = 'UPDATE_PAYPAL_SITE_CAS_QUEUE'; #守护进程名称
